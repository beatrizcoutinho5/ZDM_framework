...
if import_test_train_data == 1:
    ...
    if augmentation == 1:
        ...
        x_train = pd.read_excel(
            r'..\data\split_train_test_data\multiclass\defect_codes\x_train_aug.xlsx')

        y_train = pd.read_excel(
            r'..\data\split_train_test_data\multiclass\defect_codes\y_train_aug.xlsx')['Defect Code']

    else:

        x_train = pd.read_excel(
            r'..\data\split_train_test_data\multiclass\defect_codes\x_train.xlsx')

        y_train = pd.read_excel(
            r'..\data\split_train_test_data\multiclass\defect_codes\y_train.xlsx')['Defect Code']

    x_test = pd.read_excel(r'..\data\split_train_test_data\multiclass\defect_codes\x_test.xlsx')

    y_test = pd.read_excel(r'..\data\split_train_test_data\multiclass\defect_codes\y_test.xlsx')[
        'Defect Code']

    print(f'Loaded test and train data!')

# Configure logging file
logging.basicConfig(filename=f'logs/defect_codes_prediction_model_metrics_log.txt', level=logging.INFO)
logging.info(f"\nTimestamp: {timestamp}")
...
if import_test_train_data == 0:
    ...
    df_train = df_train[df_train["Defect Code"].isin(top_defects)]
    ...
    if augmentation == 1:
        ...
        if make_plots == 1:
            ...
            output_path = os.path.join(aug_plots_dir, 'defect_codes_before_and_after.png')
    ...
    if augmentation == 1:
        ...
        pd.Series(y_train, name='Defect Code').to_excel(
            r'..\data\split_train_test_data\multiclass\defect_codes\y_train_aug.xlsx',
            index=False)
        ...
        pd.Series(y_train, name='Defect Code').to_excel(
            r'..\data\split_train_test_data\multiclass\defect_codes\y_train.xlsx',
            index=False)

    pd.DataFrame(x_test, columns=column_names).to_excel(
        r'..\data\split_train_test_data\multiclass\defect_codes\x_test.xlsx',
        index=False)

    pd.Series(y_test, name='Defect Code').to_excel(
        r'..\data\split_train_test_data\multiclass\defect_codes\y_test.xlsx',
        index=False)
...
if load_models == 1:
    rndforest = load(r'models/multiclass/defect_codes/random_forest_model.pkl')
else:
...
plt.savefig(os.path.join(r'..\plots\confusion_matrix\multiclass\defect_codes', 'rf.png'))
plt.show()
...
# Save
if load_models == 0:
    dump(rndforest, r'models/multiclass/defect_codes/random_forest_model.pkl')
...
if load_models == 1:
    xgb_model.load_model(r'models/multiclass/defect_codes/xgb_model.json')
else:
...
confusion_matrix_xgb = confusion_matrix(y_test_original, y_pred_xgb)
disp_xgb = ConfusionMatrixDisplay(confusion_matrix=confusion_matrix_xgb, display_labels=label_encoder.classes_)
disp_xgb.plot(cmap='GnBu')
plt.title('XGBoost Confusion Matrix')
plt.savefig(os.path.join(r'..\plots\confusion_matrix\multiclass\defect_codes', 'xgboost.png'))
plt.show()
...
# Save
if load_models == 0:
    xgb_model.save_model(r'models/multiclass/defect_codes/xgb_model.json')
...
if load_models == 1:
    catboost_model = CatBoostClassifier(loss_function='MultiClass', verbose=False)
    catboost_model.load_model(r'models/multiclass/defect_codes/catboost_model.cbm')
else:
...
plt.savefig(os.path.join(r'..\plots\confusion_matrix\multiclass\defect_codes', 'catboost.png'))
plt.show()
...
# Save
if load_models == 0:
    catboost_model.save_model(r'models/multiclass/defect_codes/catboost_model.cbm')
...
#
# if load_models == 1:
#     voting_classifier = load(r'models/multiclass/defect_codes/voting_model.pkl')
#
# else:
...
plt.savefig(os.path.join(r'..\plots\confusion_matrix\multiclass\defect_codes', 'ensemble.png'))
plt.show()
...
# Save
if load_models == 0:
    dump(voting_classifier, r'models/multiclass/defect_codes/voting_model.pkl')
...
#
# if load_models == 1:
#     svm_model = load(r'models/multiclass/defect_codes/svm_model.pkl')
# else:
...
# plt.savefig(r'..\plots\confusion_matrix\multiclass\defect_codes\svm.png')
# plt.show()
#
...
# # Save
# if load_models == 0:
#     dump(svm_model, r'/multiclass/defect_codes/svm_model.pkl')
