...
if import_test_train_data == 1:
    ...
    x_train_aug = pd.read_excel(
        r'dev\data\split_train_test_data\without_delta_values\binary_data\binary_x_train_aug.xlsx')

    y_train_aug = pd.read_excel(
        r'dev\data\split_train_test_data\without_delta_values\binary_data\binary_y_train_aug.xlsx')[
        'Defect Code']

    x_test = pd.read_excel(
        r'dev\data\split_train_test_data\without_delta_values\binary_data\binary_x_test.xlsx')

    final_y_test = pd.read_excel(
        r'dev\data\split_train_test_data\without_delta_values\binary_data\binary_y_test.xlsx')[
        'Defect Code']
...
if import_test_train_data == 0:
    ...
    df_line410_2022 = pd.read_excel(r'dev\data\clean_data\binary_cleaned_data_2022_line410.xlsx')
    ...
    #     r'dev\data\split_train_test_data\without_delta_values\binary_data\binary_scaling_param.xlsx',
    #     index=False)
...
plt.savefig(r'dev\plots\confusion_matrix\without_delta_values\binary\rf.png')
plt.show()
...
if load_models == 1:
    xgb_model.load_model(r'dev\models\without_delta_values\binary\xgb_model.model')
else:
...
plt.savefig(r'dev\plots\confusion_matrix\without_delta_values\binary\xgboost.png')
plt.show()
...
# Save
if load_models == 0:
    xgb_model.save_model(r'dev\models\without_delta_values\binary\binary_xgb_model.json')
...
#
# if load_models == 1:
#     svm_model = load(r'dev\models\without_delta_values\binary\binary_svm_model.pkl')
# else:
...
# plt.savefig(r'dev\plots\confusion_matrix\without_delta_values\binary\svm.png')
# plt.show()
#
...
# # Save
# if load_models == 0:
#     dump(svm_model, r'dev\models\without_delta_values\binary\binary_svm_model.pkl')
...
if load_models == 1:
    catboost_model = CatBoostClassifier(loss_function='Logloss', verbose=False)
    catboost_model.load_model(r'dev\models\without_delta_values\binary\binary_catboost_model.cbm')
else:
...
plt.savefig(r'dev\plots\confusion_matrix\without_delta_values\binary\catboost.png')
plt.show()
...
# Save
if load_models == 0:
    catboost_model.save_model(r'dev\models\without_delta_values\binary\binary_catboost_model.cbm')
