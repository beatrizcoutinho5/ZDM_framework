...
if import_test_train_data == 1:
    ...
    if augmentation == 1:
        ...
        x_train = pd.read_excel(
            r'..\data\split_train_test_data\multiclass\defect_categories\x_train_aug.xlsx')

        y_train = pd.read_excel(
            r'..\data\split_train_test_data\multiclass\defect_categories\y_train_aug.xlsx')['Group']

    else:

        x_train = pd.read_excel(
            r'..\data\split_train_test_data\multiclass\defect_categories\x_train.xlsx')

        y_train = pd.read_excel(
            r'..\data\split_train_test_data\multiclass\defect_categories\y_train.xlsx')['Group']

    x_test = pd.read_excel(r'..\data\split_train_test_data\multiclass\defect_categories\x_test.xlsx')

    y_test = pd.read_excel(r'..\data\split_train_test_data\multiclass\defect_categories\y_test.xlsx')[
        'Group']
...
if import_test_train_data == 0:
    ...
    if augmentation == 1:
        ...
        cat_order = ["Erro Papel", "Outros", "Erro Prensa", "Manchas e Marcas", "Erro Tablero", "Pruebas", "Puntos"]
        defect_count_aug = defect_count_aug.reindex(cat_order)
        ...
        if make_plots == 1:
            ...
            sns.barplot(x=defect_count.index, y=defect_count.values, palette="autumn", ax=axes[0])
            axes[0].set_title("Quantity of the different defect codes in train data - before SMOTE")
            axes[0].set_xlabel("Group")
            axes[0].set_ylabel("Quantity")

            axes[0].set_xticklabels(defect_count.index, rotation=45, ha="right")


            # Number of occurrences for each class of defects after augmentation
            sns.barplot(x=defect_count_aug.index, y=defect_count_aug.values, palette="autumn", ax=axes[1])
            axes[1].set_title("Quantity of the different defect codes in train data - after SMOTE")
            axes[1].set_xlabel("Group")
            axes[1].set_ylabel("Quantity")

            axes[1].set_xticklabels(defect_count_aug.index, rotation=45, ha="right")

            output_path = os.path.join(aug_plots_dir, 'defect_categories_before_and_after.png')
    ...
    if augmentation == 1:
        ...
        pd.Series(y_train, name='Group').to_excel(
            r'..\data\split_train_test_data\multiclass\defect_categories\y_train_aug.xlsx',
            index=False)
        ...
        pd.Series(y_train, name='Group').to_excel(
            r'..\data\split_train_test_data\multiclass\defect_categories\y_train.xlsx',
            index=False)

    pd.DataFrame(x_test, columns=column_names).to_excel(
        r'..\data\split_train_test_data\multiclass\defect_categories\x_test.xlsx',
        index=False)

    pd.Series(y_test, name='Group').to_excel(
        r'..\data\split_train_test_data\multiclass\defect_categories\y_test.xlsx',
        index=False)
...
if load_models == 1:
    rndforest = load(r'models/multiclass/defect_categories/random_forest_model.pkl')
else:
...
plt.tight_layout()
plt.savefig(os.path.join(r'..\plots\confusion_matrix\multiclass\defect_categories', 'rf.png'))
plt.show()
...
# Save
if load_models == 0:
    dump(rndforest, r'models/multiclass/defect_categories/random_forest_model.pkl')
...
if load_models == 1:
    xgb_model.load_model(r'models/multiclass/defect_categories/xgb_model.json')
else:
...
plt.savefig(os.path.join(r'..\plots\confusion_matrix\multiclass\defect_categories', 'xgboost.png'))
plt.show()
...
# Save
if load_models == 0:
    xgb_model.save_model(r'models/multiclass/defect_categories/xgb_model.json')
...
if load_models == 1:
    catboost_model = CatBoostClassifier(loss_function='MultiClass', verbose=False)
    catboost_model.load_model(r'models/multiclass/defect_categories/catboost_model.cbm')
else:
...
disp_cat.plot(cmap='GnBu')
plt.title('Catboost Confusion Matrix')
plt.savefig(os.path.join(r'..\plots\confusion_matrix\multiclass\defect_categories', 'catboost.png'))
plt.show()
...
# Save
if load_models == 0:
    catboost_model.save_model(r'models/multiclass/defect_categories/catboost_model.cbm')
...
#
# if load_models == 1:
#     voting_classifier = load(r'models/multiclass/defect_categories/voting_model.pkl')
#
# else:
...
# disp_en.plot(cmap='GnBu')
# plt.title('Ensemble Model Confusion Matrix')
# plt.savefig(os.path.join(r'..\plots\confusion_matrix\multiclass\defect_categories', 'ensemble.png'))
# plt.show()
...
# Save
if load_models == 0:
    dump(voting_classifier, r'models/multiclass/defect_categories/voting_model.pkl')
...
#
# if load_models == 1:
#     svm_model = load(r'models/multiclass/defect_categories/svm_model.pkl')
# else:
...
# plt.savefig(os.path.join(r'..\plots\confusion_matrix\multiclass\defect_categories', 'svm.png'))
# plt.show()
#
...
# # Save
# if load_models == 0:
#     dump(svm_model, r'models/multiclass/defect_categories/svm_model.pkl')
