...
if import_test_train_data == 1:
    ...
    if augmentation == 1:
        ...
        x_train = pd.read_excel(
            r'..\data\split_train_test_data\defect_groups\defect_groups_0_x_train_aug.xlsx')

        y_train = pd.read_excel(
            r'..\data\split_train_test_data\defect_groups\defect_groups_0_y_train_aug.xlsx')['Group']

    else:

        x_train = pd.read_excel(
            r'..\data\split_train_test_data\defect_groups\defect_groups_0_x_train.xlsx')

        y_train = pd.read_excel(
            r'..\data\split_train_test_data\defect_groups\defect_groups_0_y_train.xlsx')['Group']

    x_test = pd.read_excel(r'..\data\split_train_test_data\defect_groups\defect_groups_0_x_test.xlsx')

    y_test = pd.read_excel(r'..\data\split_train_test_data\defect_groups\defect_groups_0_y_test.xlsx')[
        'Defect Code']

    print(f'Loaded test and train data!')

# Configure logging file
logging.basicConfig(filename=f'logs/defect_group_prediction_model_metrics_log.txt', level=logging.INFO)
logging.info(f"\nTimestamp: {timestamp}")
...
if import_test_train_data == 0:
    ...
    if augmentation == 1:
        ...
        if make_plots == 1:
            ...
            output_path = os.path.join(aug_plots_dir, 'defect_groups_0_before_and_after.png')
    ...
    if augmentation == 1:
        ...
        pd.Series(y_train, name='Defect Code').to_excel(
            r'..\data\split_train_test_data\defect_groups\defect_groups_0_y_train_aug.xlsx',
            index=False)
        ...
        pd.Series(y_train, name='Defect Code').to_excel(
            r'..\data\split_train_test_data\defect_groups\defect_groups_0_y_train.xlsx',
            index=False)

    pd.DataFrame(x_test, columns=column_names).to_excel(
        r'..\data\split_train_test_data\defect_groups\defect_groups_0_x_test.xlsx',
        index=False)

    pd.Series(y_test, name='Defect Code').to_excel(
        r'..\data\split_train_test_data\defect_groups\defect_groups_0_y_test.xlsx',
        index=False)
...
if load_models == 1:
    rndforest = load(r'models/defect_groups/defect_groups_0_random_forest_model.pkl')
else:
...
# Save
if load_models == 0:
    dump(rndforest, r'models/defect_groups/defect_groups_0_random_forest_model.pkl')
...
if load_models == 1:
    xgb_model.load_model(r'models\defect_groups\defect_groups_0_xgb_model.json')
else:
...
# Save
if load_models == 0:
    xgb_model.save_model(r'models\defect_groups\defect_groups_0_xgb_model.json')
...
#
# if load_models == 1:
#     svm_model = load(r'..\models\defect_groups\defect_groups_0_svm_model.pkl')
# else:
...
# # Save
# if load_models == 0:
#     dump(svm_model, r'..\models\defect_groups\defect_groups_0_svm_model.pkl')
...
if load_models == 1:
    catboost_model = CatBoostClassifier(loss_function='MultiClass', verbose=False)
    catboost_model.load_model(r'models\defect_groups\defect_groups_0_catboost_model.cbm')
else:
...
# Save
if load_models == 0:
    catboost_model.save_model(r'models\defect_groups\defect_groups_0_catboost_model.cbm')
...
#
# if load_models == 1:
#     voting_classifier = load(r'models\defect_groups\defect_groups_0_voting_model.pkl')
#
# else:
...
# # Save
# if load_models == 0:
#     dump(voting_classifier, r'models\defect_groups\defect_groups_0_voting_model.pkl')
