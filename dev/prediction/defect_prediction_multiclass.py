...
if import_test_train_data == 1:
    ...
    if augmentation == 1:
        ...
        x_train = pd.read_excel(
            r'..\data\split_train_test_data\multiclass\multiclass_x_train_aug.xlsx')

        y_train = pd.read_excel(
            r'..\data\split_train_test_data\multiclass\multiclass_y_train_aug.xlsx')['Defect Code']

    else:

        x_train = pd.read_excel(
            r'..\data\split_train_test_data\multiclass\multiclass_x_train.xlsx')

        y_train = pd.read_excel(
            r'..\data\split_train_test_data\multiclass\multiclass_y_train.xlsx')['Defect Code']

    x_test = pd.read_excel(r'..\data\split_train_test_data\multiclass\multiclass_x_test.xlsx')

    y_test = pd.read_excel(r'..\data\split_train_test_data\multiclass\multiclass_y_test.xlsx')[
        'Defect Code']

    print(f'Loaded test and train data!')

# Configure logging file
logging.basicConfig(filename=f'logs/prediction_model_metrics_log.txt', level=logging.INFO)
logging.info(f"\nTimestamp: {timestamp}")
...
if import_test_train_data == 0:
    ...
    # just the top 10 defects
    top_defects = [0, 4, 14, 27, 29, 100, 104, 105, 106, 132, 134]
    ...
    if augmentation == 1:
        ...
        if make_plots == 1:
            ...
            output_path = os.path.join(aug_plots_dir, 'multiclass_before_and_after.png')
    ...
    if augmentation == 1:
        ...
        pd.Series(y_train, name='Defect Code').to_excel(
            r'..\data\split_train_test_data\multiclass\multiclass_y_train_aug.xlsx',
            index=False)
        ...
        pd.Series(y_train, name='Defect Code').to_excel(
            r'..\data\split_train_test_data\multiclass\multiclass_y_train.xlsx',
            index=False)

    pd.DataFrame(x_test, columns=column_names).to_excel(
        r'..\data\split_train_test_data\multiclass\multiclass_x_test.xlsx',
        index=False)

    pd.Series(y_test, name='Defect Code').to_excel(
        r'..\data\split_train_test_data\multiclass\multiclass_y_test.xlsx',
        index=False)
...
if load_models == 1:
    rndforest = load(r'models/multiclass/multiclass_random_forest_model.pkl')
else:
...
plt.savefig(r'..\plots\confusion_matrix\multiclass\rf.png')
plt.show()
...
# Save
if load_models == 0:
    dump(rndforest, r'models/multiclass/multiclass_random_forest_model.pkl')
...
if load_models == 1:
    xgb_model.load_model(r'models/multiclass/multiclass_xgb_model.json')
else:
...
plt.savefig(r'..\plots\confusion_matrix\multiclass\xgboost.png')
plt.show()
...
# Save
if load_models == 0:
    xgb_model.save_model(r'models\multiclass\multiclass_xgb_model.json')
...
#
# if load_models == 1:
#     svm_model = load(r'..\models\multiclass\multiclass_svm_model.pkl')
# else:
...
# plt.savefig(r'..\plots\confusion_matrix\multiclass\svm.png')
# plt.show()
#
...
# # Save
# if load_models == 0:
#     dump(svm_model, r'..\models\multiclass\multiclass_svm_model.pkl')
...
if load_models == 1:
    catboost_model = CatBoostClassifier(loss_function='MultiClass', verbose=False)
    catboost_model.load_model(r'models\multiclass\multiclass_catboost_model.cbm')
else:
...
plt.savefig(r'..\plots\confusion_matrix\multiclass\catboost.png')
plt.show()
...
# Save
if load_models == 0:
    catboost_model.save_model(r'models\multiclass\multiclass_catboost_model.cbm')
...
#
# if load_models == 1:
#     voting_classifier = load(r'models\multiclass\multiclass_voting_model.pkl')
#
# else:
...
# plt.savefig(os.path.join(r'..\plots\confusion_matrix\multiclass', 'ensemble.png'))
# plt.show()
#
...
# # Save
# if load_models == 0:
#     dump(voting_classifier, r'models\multiclass\multiclass_voting_model.pkl')
