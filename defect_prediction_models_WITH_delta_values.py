...
if load_models == 1:
    rndforest = load(r'models\with_delta_values\multiclass\random_forest_model.pkl')
else:
...
if load_models == 1:
    xgb_model.load_model(r'models\with_delta_values\multiclass\xgb_model.json')
else:
...
if load_models == 1:
    catboost_model = CatBoostClassifier(loss_function='MultiClass', verbose=False)
    catboost_model.load_model(r'models\with_delta_values\multiclass\catboost_model.cbm')
else:
...
confusion_matrix_cat = confusion_matrix(final_y_test, y_pred_cat, labels=labels_cat)
